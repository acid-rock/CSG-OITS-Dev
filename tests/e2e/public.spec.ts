...
test.describe('Public homepage', () => {
  ...
  test('clicking Announcements in dropdown navigates to /announcements', async ({ page }) => {
    await page.goto('/');
    await page.hover('text=News');
...
test.describe('/borrow equipment page', () => {
  ...
  test('Request to Borrow button opens form', async ({ page }) => {
    await page.goto('/borrow');
    const borrowBtn = page.locator('button', { hasText: 'Request to Borrow' }).first();
    if (await borrowBtn.isVisible()) {
      await borrowBtn.click();
      await expect(page.locator('input[type="email"], [placeholder*="email"]'))
