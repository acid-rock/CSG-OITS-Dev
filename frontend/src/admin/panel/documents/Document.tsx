...
const Documents = () => {
    ...
    axios
      ...
      .then(({ data }) => {
        const map: Record<string, { full_name: string | null; email: string }> =
          {};
        for (const u of data)
          map[u.owner_id] = { full_name: u.full_name ?? null, email: u.email };
        setAdminMap(map);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    setCounts((p) => ({ ...p, [tab]: data.length }));
  }, [data, tab]);
  useEffect(() => {
    (["active", "archived", "bin"] as const)
      .filter((t) => t !== tab)
      .forEach((t) => {
        const ep =
          t === "archived"
            ...
            : t === "bin"
              ...
              : `${API_URL}/documents`;
        axios
          .get(ep, { withCredentials: true })
          .then(({ data: r }) => {
            const rows: DocumentEntry[] = Array.isArray(r)
              ? r
              : (((r as Record<string, unknown>).data as DocumentEntry[]) ??
                []);
  ...
  const aPageRows = filteredActive.slice(
    (aSafePage - 1) * PAGE_SIZE,
    aSafePage * PAGE_SIZE,
  );
  const aFrom =
    filteredActive.length === 0 ? 0 : (aSafePage - 1) * PAGE_SIZE + 1;
  ...
  const bPageRows = data.slice(
    (bSafePage - 1) * PAGE_SIZE,
    bSafePage * PAGE_SIZE,
  );
              ...
              <section className="ad-card">
                        ...
                        <input
                          ...
                          onChange={() =>
                            setActive(
                              aPageRows.every((e) => active.includes(e.id))
                                ? active.filter(
                                    (id) => !aPageRows.find((e) => e.id === id),
                                  )
                                : [
                                    ...new Set([
                                      ...active,
                                      ...aPageRows.map((e) => e.id),
                                    ]),
                                  ],
                            )
                          }
                  ...
                  <tbody>
                    {aPageRows.map((entry) => (
                      <tr
                        key={entry.id}
                        className={
                          active.includes(entry.id) ? "is-selected" : ""
                        }
                      >
                        <td>
                          <input
                          ...
                          <div className="ad-title-stack">
                            <div className="ad-title-row">
                              <Tag
                                ...
                                tone="primary"
                              />
                              <span className="ad-title-link ad-mono">
                                {entry.name}
                              </span>
                            </div>
                            ...
                            <div className="ad-meta">
                              {entry.term && (
                                <span className="ad-mono">{entry.term}</span>
                              )}
                            </div>
                          </div>
                        </td>
                        <td>
                          <div className="ad-author">
                            <MiniAvatar
                              name={adminDisplayName(entry.owner_id, adminMap)}
                            />
                            <span>
                              {adminDisplayName(entry.owner_id, adminMap)}
                            </span>
                          </div>
                        </td>
                        <td>
                ...
                <div className="ad-table-foot">
                  <span className="ad-foot-count">
                    ...
                    of <strong>{filteredActive.length}</strong> documents
                  </span>
                  {aTotalPages > 1 && (
                    <div className="ad-foot-pager">
                      <button
                        className="ad-page-btn"
                        disabled={aSafePage <= 1}
                      ...
                      {Array.from({ length: aTotalPages }, (_, i) => i + 1)
                        ...
                        .map((n, i) =>
                          n === "…" ? (
                            ...
                            <button
                              key={n}
                              className={`ad-page-btn${n === aSafePage ? " is-active" : ""}`}
                      ...
                      <button
                        className="ad-page-btn"
                        disabled={aSafePage >= aTotalPages}
                        onClick={() =>
                          setPage((p) => Math.min(aTotalPages, p + 1))
                        }
                      >
                        Next ›
                      </button>
                        ...
                        <thead>
                          <tr>
                            <th>
                              <input
                                type="checkbox"
                                title="Select all in term"
                                checked={
                                  archivedGroups[term].length > 0 &&
                                  archivedGroups[term].every((e) =>
                  ...
                  {bTotalPages > 1 && (
                    <div className="ad-foot-pager">
                      ...
                      {Array.from({ length: bTotalPages }, (_, i) => i + 1)
                        ...
                        .map((n, i) =>
                          n === "…" ? (
                            ...
                            <button
                              key={n}
                              className={`ad-page-btn${n === bSafePage ? " is-active" : ""}`}
                              onClick={() => setPage(n as number)}
                            >
                              {n}
                            </button>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setPage((p) => Math.min(bTotalPages, p + 1))
                        }
                      >
                        Next ›
                      </button>
