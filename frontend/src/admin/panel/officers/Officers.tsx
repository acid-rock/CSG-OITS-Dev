...
const groupByTerm = (items: OfficerEntry[]): Record<string, OfficerEntry[]> => {
  ...
  items.forEach((item) => {
    const term =
      item.year_serving ?? academicYear(item.archived_at ?? undefined);
...
const OfficersPanel = () => {
  ...
  const fetchInitial = useCallback(async () => {
    ...
    if (termRes.status === "fulfilled")
      setActiveTerm(termRes.value.data?.value ?? "");
    ...
    async (t: Tab) => {
      ...
      const ep =
        t === "archived"
          ...
          : `${API_URL}/officers/bin`;
      try {
        const { data: rows } = await axios.get<OfficerEntry[]>(ep, {
          withCredentials: true,
        });
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [actRes, arcRes, binRes, comRes, termRes] = await Promise.allSettled([
      ...
      axios.get<OfficerEntry[]>(`${API_URL}/officers/archived`, {
        withCredentials: true,
      }),
      axios.get<OfficerEntry[]>(`${API_URL}/officers/bin`, {
        withCredentials: true,
      }),
      axios.get<Committee[]>(`${API_URL}/committees`, {
        withCredentials: true,
  ...
  const data =
    tab === "archived" ? archivedData : tab === "bin" ? binData : activeData;
  const counts = {
  ...
  const handleActive = (eid: string) =>
    setActive((p) =>
      p.includes(eid) ? p.filter((a) => a !== eid) : [...p, eid],
    );
  const handleRefresh = () => {
    setSpinning(true);
    fetchAll().finally(() => {
  ...
  const handleRestore = async (officerId: string) => {
    try {
      ...
      if (tab === "archived")
        setArchivedData((p) => p.filter((o) => o.id !== officerId));
    } catch (err: unknown) {
      const d = (
        err as { response?: { data?: { error?: string; message?: string } } }
      )?.response?.data;
  ...
  const handleDelete = async (officerId: string, name: string) => {
    if (tab === "active") {
      ...
      try {
        ...
        setFetchError("Move to bin failed.");
      }
    } else {
      ...
      try {
        ...
        if (tab === "archived")
          setArchivedData((p) => p.filter((o) => o.id !== officerId));
        if (tab === "bin")
          setBinData((p) => p.filter((o) => o.id !== officerId));
        setActive((p) => p.filter((a) => a !== officerId));
      } catch {
    ...
    try {
      await Promise.all(
        active.map((id) =>
          axios.post(
            `${API_URL}/officers/archive`,
            { id, term_year: activeTerm || academicYear() },
            { withCredentials: true },
          ),
        ),
      );
      ...
      setFetchError("Bulk archive failed.");
    }
  };
    ...
    try {
      const ep =
        tab === "bin"
          ...
          : `${API_URL}/officers/restore`;
      await Promise.all(
        active.map((id) => axios.post(ep, { id }, { withCredentials: true })),
      );
      fetchAll();
      setActive([]);
    } catch {
      setFetchError("Bulk restore failed.");
    }
  };

  const bulkDelete = async () => {
    if (!active.length || !window.confirm(`Delete ${active.length} officers?`))
      return;
    try {
      await axios.delete(`${API_URL}/officers/delete`, {
        data: { ids: active },
      ...
      setFetchError("Bulk delete failed.");
    }
  };
  ...
  const isTermOpen = (t: string) =>
    openTerms[t] !== undefined ? openTerms[t] : t === sortedTerms[0];
  ...
  const filteredData = data
    .filter(
      (o) =>
        !filter ||
        filter === "All" ||
        o.type?.toLowerCase() === filter.toLowerCase(),
    )
    .filter(
      (o) =>
        !searchQuery ||
  ...
  const ORow = ({ o }: { o: OfficerEntry }) => {
    ...
    return (
      <tr className={active.includes(o.id) ? "is-selected" : ""}>
        ...
        <td>
          <span className="ad-mono ad-cell-muted">{o.year_serving ?? "—"}</span>
        </td>
      ...
      <section className="ad-card">
            ...
            <tr>
              <th>
                <input
                  ...
                  checked={
                    pageRows.length > 0 &&
                    pageRows.every((o) => active.includes(o.id))
                  }
                  onChange={() =>
                    setActive(
                      pageRows.every((o) => active.includes(o.id))
                        ? active.filter(
                            (id) => !pageRows.find((o) => o.id === id),
                          )
                        : [
                            ...new Set([
                              ...active,
        ...
        <div className="ad-table-foot">
          <span className="ad-foot-count">
            ...
            of <strong>{rows.length}</strong> officers
          </span>
          {totalPages > 1 && (
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((n, i) =>
                  n === "…" ? (
                    ...
                    <button
                      key={n}
                      className={`ad-page-btn${n === safePage ? " is-active" : ""}`}
        ...
        <main className="ad-main">
          <PageHead
            ...
            actions={
              <>
                ...
                <button className="ad-btn-ghost" onClick={handleExportCSV}>
            ...
            items={[
              {
                label: "Active",
                active: tab === "active",
                count: counts.active || undefined,
              },
              {
                label: "Archived",
                active: tab === "archived",
                count: counts.archived || undefined,
              },
              {
                label: "Bin",
                active: tab === "bin",
                count: counts.bin || undefined,
              },
            ]}
          ...
          {tab === "active" && (
            <BulkBar
              ...
              handlers={{ Archive: bulkArchive, Delete: bulkDelete }}
              onClear={() => setActive([])}
            />
          )}
          {(tab === "archived" || tab === "bin") && (
            <BulkBar
              ...
              handlers={{ Restore: bulkRestore, Delete: bulkDelete }}
              onClear={() => setActive([])}
            />
          )}
                ...
                >
                  ...
                  >
                    ...
                    <span className="ad-term-meta">
                      {archivedGroups[term].length} officer
                      {archivedGroups[term].length !== 1 ? "s" : ""}
                      <I.chev
                        width="13"
                        height="13"
                        style={{
                          transform: isTermOpen(term)
                            ? "rotate(180deg)"
                            : "none",
                  ...
                  {isTermOpen(term) && (
                    <OfficerTable
                      rows={archivedGroups[term]}
                      empty="No officers in this term."
                    />
                  )}
                </section>
              ))}
