...
const Audit = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const { data: r } = await axios.get<AuditEntry[]>(
        `${API_URL}/auditlog/`,
  ...
  useEffect(() => {
    setPage(1);
  }, [selectedAction, dateFrom, dateTo, searchQuery, sort]);
  ...
  const extraTypes = Array.from(
    new Set(data.map((e) => e.action).filter(Boolean)),
  )
    .filter((a) => !FIXED_ACTION_TYPES.includes(a))
    .sort();
  ...
  const filtered = data
    ...
    .filter(
      (e) =>
        !dateTo || new Date(e.created_at) <= new Date(dateTo + "T23:59:59"),
    )
    .filter(
      (e) =>
        !searchQuery ||
    ...
    <div className="ad-shell">
      ...
      <main className="ad-main">
        ...
        <div className="ad-audit-summary">
          ...
          <div className="ad-audit-stat">
            <span
              className={`ad-audit-stat-num${deletions7d > 0 ? " is-danger" : ""}`}
            >
              {deletions7d}
            </span>
        ...
        >
          ...
          <input
            ...
            style={{ width: 150 }}
          />
          <span className="ad-filter-label" style={{ padding: "0 2px" }}>
            →
          </span>
          <input
            type="date"
        ...
        {loading ? (
          <section className="ad-card">
            <div className="ad-empty">
              <p>
                Loading audit log… this may take a moment for large datasets.
              </p>
            </div>
          </section>
        ) : (
          <section className="ad-card">
                  ...
                  <tr key={entry.id ?? i}>
                    <td>
                      <div className="ad-author">
                        <MiniAvatar
                          name={entry.admin_name || entry.created_by}
                        />
                    ...
                    <td>
                      <span className="ad-mono ad-cell-text">
                        {entry.entity ?? "—"}
                      </span>
                    </td>
                    <td>
                      <span
                        className="ad-mono ad-cell-muted"
            ...
            <div className="ad-table-foot">
              <span className="ad-foot-count">
                Showing <strong>{shownLabel}</strong> of{" "}
