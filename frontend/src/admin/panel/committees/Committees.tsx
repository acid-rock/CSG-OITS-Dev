...
const CommitteesPanel = () => {
    ...
    try {
      const endpoint =
        tab === "archived"
          ...
          : `${API_URL}/committees`;
      const { data: res } = await axios.get<CommitteeEntry[]>(endpoint, {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAddError(
        err instanceof Error ? err.message : "Failed to add committee.",
      );
    } finally {
      setAdding(false);
    }
  ...
  const handleArchive = async (id: string) => {
    try {
      ...
      const msg =
        (err as { response?: { data?: { message?: string } } })?.response?.data
          ?.message ?? "Failed to archive committee.";
      setFetchError(msg);
    }
  };
  ...
  const handlePermanentDelete = async (id: string) => {
    if (
      !window.confirm(
        "Permanently delete this committee? This cannot be undone.",
      )
    )
      return;
    try {
  ...
  return (
    <div className="announce-container">
      ...
      <div className="announce-toolbar">
        ...
        {addError && (
          <span style={{ color: "red", fontSize: "0.85rem" }}>{addError}</span>
        )}
        <button
      ...
      {tab === "bin" && (
        <div style={{ padding: "2rem 1rem", textAlign: "center" }}>
          ...
          >
            Bin functionality requires a database migration. Please contact your
                ...
                <th style={{ width: "40px", padding: "0.5rem" }}>
                  {tab === "active" && (
                    <input
                      ...
                      onChange={(e) =>
                        setSelectedIds(
                          e.target.checked ? data.map((c) => String(c.id)) : [],
                        )
                      }
                    />
                  )}
                    ...
                    {tab === "active" && (
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedIds((prev) =>
                            e.target.checked
                              ? [...prev, String(entry.id)]
                              : prev.filter((id) => id !== String(entry.id)),
                          )
                        }
                      />
                    )}
