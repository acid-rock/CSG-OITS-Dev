...
function apiErr(err: unknown, fallback: string): string {
  ...
  return (
    d?.error ?? d?.message ?? (err instanceof Error ? err.message : fallback)
  );
}
...
function MemberAutocomplete({
  ...
  const filtered =
    query.trim().length > 0
      ? available
          .filter((o) =>
            o.full_name.toLowerCase().includes(query.trim().toLowerCase()),
          )
          .slice(0, 8)
      : [];
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative" }}>
      <input
        ...
        onFocusCapture={(e) =>
          (e.currentTarget.style.borderColor = "var(--color-primary,#4f6fd1)")
        }
        onBlurCapture={(e) => {
          if (!open)
...
function MembersSection({
  ...
  const currentMembers = allOfficers.filter(
    (o) =>
      (committeeId &&
        String(o.committee) === committeeId &&
        !pendingRemove.includes(o.id)) ||
      pendingAdd.includes(o.id),
  );

  const available = allOfficers.filter(
    (o) =>
          ...
          {currentMembers.map((m) => (
            <span
              ...
              style={{
                ...
                fontSize: 12,
                padding: "4px 8px",
                background: "rgba(79,111,209,0.08)",
...
function EditDetailsModal({
  ...
  const [coverPreview, setCoverPreview] = useState<string | null>(
    committee.cover_image_url ?? null,
  );
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pendingAdd.length || pendingRemove.length) {
        await axios.post(
          `${API_URL}/committees/update-members`,
          {
        ...
        <form onSubmit={handleSave} className="ad-form-body">
          ...
          <div className="ad-field">
            ...
            >
              ...
              <span className="ad-upload-badge">
                {coverPreview ? "Click to replace" : "PNG, JPG"}
              </span>
            </div>
            <input
              ref={fileRef}
          ...
          <label className="ad-field">
            ...
            <input
              ...
              required
              placeholder="e.g. Committee on Academic Affairs"
            />
          </label>
          ...
          <div className="ad-form-actions">
            <button type="button" className="ad-btn-ghost" onClick={onClose}>
              Cancel
            </button>
...
function AddCommitteeModal({
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(apiErr(err, "Failed to add."));
    } finally {
      setAdding(false);
    }
  };
  ...
  const fakeMembersForPending = allOfficers.filter((o) =>
    pendingAdd.includes(o.id),
  );
      ...
      >
        ...
        <form onSubmit={handleAdd} className="ad-form-body">
                ...
                {fakeMembersForPending.map((m) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setPendingAdd((p) => p.filter((id) => id !== m.id))
                      }
                      style={{
                        background: "none",
                        border: "none",
          ...
          <div className="ad-form-actions">
            ...
            <button type="submit" className="ad-btn-primary" disabled={adding}>
...
const CommitteesPanel = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [committeesRes, officersRes] = await Promise.all([
        ...
        axios.get<OfficerEntry[]>(`${API_URL}/officers`, {
          withCredentials: true,
        }),
      ]);
  ...
  const handleArchive = async (id: string) => {
    try {
      await axios.post(
        `${API_URL}/committees/archive`,
        { ids: [id] },
        { withCredentials: true },
      );
      setData((p) => p.filter((c) => c.id !== id));
    } catch (err: unknown) {
      setFetchError(apiErr(err, "Archive failed."));
    }
  };
  const handleRestore = async (id: string) => {
    try {
      await axios.post(
        `${API_URL}/committees/restore`,
        { ids: [id] },
        { withCredentials: true },
      );
      fetchData();
    } catch (err: unknown) {
      setFetchError(apiErr(err, "Restore failed."));
    }
  };
  const handleMoveToBin = async (id: string) => {
    ...
    if (count > 0) {
      setFetchError(
        `Cannot move to bin: this committee has ${count} active member(s). Reassign them first.`,
      );
      return;
    }
    ...
    try {
      ...
      setFetchError(apiErr(err, "Move to bin failed."));
    }
  };
  const handleDelete = async (id: string) => {
    if (!window.confirm("Permanently delete? This cannot be undone.")) return;
    try {
      await axios.delete(`${API_URL}/committees/delete`, {
        data: { ids: [id] },
        withCredentials: true,
      });
      setData((p) => p.filter((c) => c.id !== id));
    } catch (err: unknown) {
      setFetchError(apiErr(err, "Delete failed."));
    }
  };
  const handleRestoreFromBin = async (id: string) => {
    try {
      await axios.post(
        `${API_URL}/committees/restore-from-bin`,
        { ids: [id] },
        { withCredentials: true },
      );
      setData((p) => p.filter((c) => c.id !== id));
  ...
  const handleBulkArchive = async () => {
    if (
      !selectedIds.length ||
      !window.confirm(`Archive ${selectedIds.length} committee(s)?`)
    )
      return;
    try {
      await axios.post(
      ...
      setFetchError(apiErr(err, "Bulk archive failed."));
    }
  };
  ...
  const resolveChair = (
    ...
    const chair = officers.find(
      (o) =>
        o.committee != null &&
        String(o.committee) === c.id &&
        o.is_committee_official,
    );
    return chair
      ? { name: chair.full_name, avatar: chair.avatar ?? null }
      : null;
  };

  const memberCount = (c: CommitteeEntry): number =>
    officers.filter((o) => o.committee != null && String(o.committee) === c.id)
      .length;

  // Commented for building purposes
        ...
        <main className="ad-main">
            ...
            items={[
              {
                label: "Active",
                active: tab === "active",
                count: tab === "active" ? data.length : undefined,
              },
              {
                label: "Archived",
                active: tab === "archived",
                count: tab === "archived" ? data.length : undefined,
              },
              {
                label: "Bin",
                active: tab === "bin",
                count: tab === "bin" ? data.length : undefined,
              },
            ]}
          ...
          {tab === "active" && (
            <BulkBar
              ...
              actions={["Archive"]}
              handlers={{ Archive: handleBulkArchive }}
                  ...
                  {filtered.map((c) => {
                    ...
                    return (
                      <tr
                        key={c.id}
                        className={
                          selectedIds.includes(c.id) ? "is-selected" : ""
                        }
                      >
                        <td>
                          <input
                        ...
                        <td>
                          <div
                            style={{
                              ...
                              borderRadius: 8,
                              overflow: "hidden",
                              background: "var(--color-bg-subtle,#f8f9fc)",
                            }}
                          >
                            {c.cover_image_url ? (
                              <img
                        ...
                        <td>
                          <span className="ad-pill-stat">
                            {memberCount(c)}{" "}
                            {memberCount(c) === 1 ? "member" : "members"}
                          </span>
                        </td>
                          ...
                          {tab === "bin" && (
                            <>
                              <button
                                className="ad-icon-btn is-on"
                                title="Restore"
                                onClick={() => handleRestoreFromBin(c.id)}
                              >
                                <I.restore width="14" height="14" />
                              </button>
                              <button
