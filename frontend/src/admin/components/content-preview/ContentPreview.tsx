...
export default function ContentPreview({ type, formValues, imagePreviewUrl }: ContentPreviewProps) {
  return (
    <div className='cp-panel'>
      ...
      {type === 'announcement' && (
        <div className='cp-announcement-card'>
          {imagePreviewUrl ? (
            <img src={imagePreviewUrl} alt='Preview' className='cp-card-image' />
          ) : (
          ...
          <div className='cp-card-body'>
            ...
            <p className='cp-card-content'>
              {truncate(formValues.description, 150) || <em>Content will appear here…</em>}
            </p>
          </div>
        </div>
      )}
        ...
        <div className='cp-document-card'>
          ...
          <div className='cp-doc-body'>
            <p className='cp-doc-type'>
              {(formValues.docType || 'document').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
            </p>
            <p className='cp-doc-name'>
              {formValues.title || <em>Document name will appear here</em>}
            </p>
            {formValues.description && (
              <p className='cp-doc-description'>{truncate(formValues.description, 100)}</p>
            )}
            {formValues.pdfName && (
          ...
          <div className='cp-doc-action'>
            <button disabled className='cp-doc-btn'>View →</button>
          </div>
          <p className='cp-doc-note'>
            The uploaded PDF will be viewable inline. Certain sections may be redacted before publishing.
          </p>
        </div>
      )}
      ...
      {type === 'event' && (
        <div className='cp-event-card'>
          {imagePreviewUrl ? (
            <img src={imagePreviewUrl} alt='Preview' className='cp-event-image' />
          ) : (
          ...
          <div className='cp-event-body'>
            ...
            <p className='cp-event-description'>
              {truncate(formValues.description, 120) || <em>Description will appear here…</em>}
            </p>
          </div>
        </div>
      )}
